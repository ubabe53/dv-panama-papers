...
pp = read_csv('csv_panama_papers/panama_papers.nodes.entity.csv')

pp["incorporation_date"] = to_datetime(pp["incorporation_date"])
pp['year'], pp['month'] = pp['incorporation_date'].dt.year, pp['incorporation_date'].dt.month

pp.drop(["name", "inactivation_date", "struck_off_date", "ibcRUC", "status",
        "company_type", "valid_until", "note", "closed_date"], axis=1, inplace=True)
...
havens_full_list = ['SAM', 'PMA', 'NEV', 'UK', 'SGP', 'RAK', 'IOM', 'ANG', 'SEY',
                    'NIUE', 'UY', 'BVI', 'HK', 'NZL', 'BAH', 'CYP', 'MLT', 'BLZ', 'JSY', 'WYO', 'CRI']
...
countries_full_list = ['HKG', 'TWN', 'CHN', 'CHE', 'SGP', 'BRA', 'PAN', 'WSM', 'LIE', 'ESP', 'THA', 'COL', 'JEY', 'AND', 'SYC', 'IRL', 'BEL', 'ISR', 'GIB', 'GGY', 'ARE', 'CYP', 'VEN', 'IMN', 'LBN', 'DNK', 'URY', 'JOR', 'BHS', 'GBR', 'BLZ', 'LUX', 'ECU', 'GTM', 'DEU', 'MUS', 'TUR', 'USA', 'MCO', 'EST', 'NIU', 'CZE', 'NLD', 'HUN', 'CRI', 'PRT', 'CYM', 'ZAF', 'MLT', 'NZL', 'CIV', 'DOM', 'FRA', 'ITA', 'CAN', 'RUS', 'GRC', 'SAU', 'QAT', 'MEX', 'PER', 'BMU', 'SLV', 'AUS', 'KNA', 'ARG', 'JPN', 'AUT', 'ASM', 'PRY', 'CHL', 'EGY', 'SWE', 'VGB', 'POL', 'SVN', 'PHL', 'LCA',
                       'IDN', 'HND', 'KOR', 'KWT', 'HTI', 'ZWE', 'SDN', 'NIC', 'TCA', 'LVA', 'NGA', 'UKR', 'AIA', 'KEN', 'ROU', 'VCT', 'NOR', 'BOL', 'LTU', 'VNM', 'ABW', 'BGR', 'MYS', 'FIN', 'IRN', 'LSO', 'MOZ', 'MAC', 'GHA', 'GEO', 'YEM', 'ATG', 'CUW', 'IND', 'MKD', 'MAR', 'SEN', 'DMA', 'NAM', 'BWA', 'CUB', 'LBR', 'COK', 'SYR', 'SXM', 'BLR', 'DJI', 'BRB', 'TUN', 'BHR', 'HRV', 'NRU', 'AZE', 'PAK', 'LBY', 'UGA', 'UZB', 'TTO', 'ISL', 'LKA', 'VIR', 'TZA', 'PRI', 'MWI', 'TCD', 'GUM', 'OMN', 'JAM', 'MLI', 'MNE', 'KAZ', 'VUT', 'MDA', 'AGO', 'BGD', 'CAF', 'BRN', 'ALB', 'CMR', 'ZMB', 'SVK']
...
def time_series(selected_havens, selected_year):
    ...
    for c in selected_havens:
        traces.append(dict(
            x=filt_df.year,
            y=filt_df[c],
            name=pp.loc[pp['jurisdiction'] == c,
                        'jurisdiction_description'].drop_duplicates().values[0],
        ))
...
def sankey_flow(selected_country, selected_year):
    ...
    fig = go.Figure(data=[go.Sankey(
        ...
        link=dict(
            ...
            value=[filt_df[filt_df["jurisdiction_description"] == x].count()[0]
                   for x in haven_list]
        ))])

    fig.update_layout(
        title_text="Tax Haven Popularity by Country", font_size=10)
...
def return_images(selected_country):
    if selected_country in img_map:
        ...
        fig.update_layout(
            title_text='Key Figures of selected Country',
            xaxis={
                "title": "For the selected country there are no Key Figures worth mentioning"},
            font_size=10,
            hovermode=False,
        )
...
def map(selected_country):
    ...
    clist = [[selected_country, x]
             for x in filt_df["jurisdiction_description"]]
